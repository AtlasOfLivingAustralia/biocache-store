...
/**
 * Load a CSV file into the backend table. This intended to only be used for data migration activities.
 *
...
object ImportUtil extends Tool {
  ...
  def cmd = "import"
  def desc = "Import data. Not for production use. This is for data migration only."
  ...
  def main(args: Array[String]) {
    ...
    val parser = new OptionParser(help) {
      ...
      opt("c", "columns", "<column1,column2,column3 ...>", "column headers", { columns: String => fieldsToImport = columns.split(",").toList })
      opt("sp", "separator", "column separator", "column separator for file to import", { v: String => charSeparator = v.charAt(0) })
      opt("qc", "quotechar", "quote character", "column separator for file to import", { v: String => quoteChar = v.charAt(0) })
      ...
      opt("json", "import the values as json", {
        json = true
      })
    ...
    if (parser.parse(args)) {
      ...
      filesToImport.foreach {
        if (json)
          ...
          importFile(entity, fieldsToImport, charSeparator, quoteChar, escapeChar, _, idColumnIdx, threads)
      }
      pm.shutdown
    }
  }
  ...
  def importJson(entity: String, filepath: String) {
    for (line <- scala.io.Source.fromFile(filepath).getLines()) {
     ...
     if(guid.isDefined){
       ...
       Config.persistenceManager.put(guid.get, entity, finalMap, true, false)
     }
    }
  }
  
  def importFile(entity: java.lang.String, fieldsToImport: List[String], separator: Char,  quotechar: Char, escapechar:Char, filepath: String, idColumnIdx:Int = 0, threads:Int = 1) {

    val reader = new CSVReader(new FileReader(filepath), separator, quotechar, escapechar)
    val executor = MoreExecutors.getExitingExecutorService(Executors.newFixedThreadPool(threads).asInstanceOf[ThreadPoolExecutor])
    ...
    while (currentLine != null) {
      if (counter > 0 && counter % 1000 == 0) {
        println(s"Last ID [$lastID], imported : $counter")
      }
      ...
      if (columns.length == fieldsToImport.length && StringUtils.isNotEmpty(columns(idColumnIdx))) {
        ...
        println(s"Problem line [$counter]: " + columns)
      }
      lastID = columns(idColumnIdx)
      //read next
      currentLine = reader.readNext
    }

    reader.close()
    println("Import complete. Total imported : " + counter)
  }
}
