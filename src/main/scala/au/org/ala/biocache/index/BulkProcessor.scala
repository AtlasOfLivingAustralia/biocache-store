...
object BulkProcessor extends Tool with Counter with RangeCalculator {
  ...
  def desc = "Bulk processor for regenerating indexes and reprocessing the entire cache. This operates per-node."
  ...
  def main(args: Array[String]) {
    ...
    val validActions = List("load-conservation-data")
    var forceMerge = true
    var mergeSegments = 1
    ...
    val parser = new OptionParser(help) {
      arg("<action>", "The action to perform. Supported values :  " + validActions.mkString(","), {
        v: String => action = v
      })
      ...
      opt("sw", "singlewriter", "Use one index writer instead of one writer per 'thread'. Default is " + singleWriter, {
        singleWriter = true
      })
      ...
      opt("sm", "skipMerge", "Force merge of segments. Default is " + forceMerge + ". For index only.", {
        forceMerge = false
      })
    ...
    if (action == "load-conservation-data") {
      new LoadTaxonConservationData(this, 1)
    }
    ...
    //                new LoadSamplingRunner(this, counter, startKey)
