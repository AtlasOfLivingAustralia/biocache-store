...
import au.org.ala.biocache.index.{BulkColumnExporter, Counter, RangeCalculator}
 ...
 */
object ExportFromIndexStream extends Tool with Counter with RangeCalculator  {
  ...
  def main(args: Array[String]) {
    val parser = new OptionParser(help) {
      ...
      opt("f", "filter-file", "Cassandra filter file for export-stream. File is a json file with an array of maps with " +
        ...
        "layers:false, user-qa:false, list-of-fields:id,scientificName.p, list-of-fields-subset1:id, " +
        "list-of-fields-subset2:scientificName.p metadata-q:\"species_group:Fishes\"}]", {
      ...
      opt("s", "start", "The rowKey in which to start the range. Only used with a filter-file. ", {
        v: String => start = v
      })
      opt("e", "end", "The rowKey in which to end the range. Only used with a filter-file. ", {
        v: String => end = v
      })
  ...
  def exportFromConfigFinish() {
    ...
    for (i <- 0 until filters.size()) {
      ...
      for (elem <- fieldsdb) { val m:List[Map[String, Object]] = List(Map("id" -> elem._1, "name" -> elem._2)); json = json ++ m}
