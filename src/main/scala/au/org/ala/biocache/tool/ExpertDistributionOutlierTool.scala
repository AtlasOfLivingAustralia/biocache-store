...
import au.org.ala.biocache.util.{GenericConsumer, Json, OptionParser}
...
object ExpertDistributionOutlierTool extends Tool {
  ...
  def main(args: Array[String]) {
    ...
    val parser = new OptionParser(help) {
      ...
      opt("test", "Test the outliers but don't write to Cassandra", {
        test = true
      })
        ...
        if (roundedDistance > 0) {
          ...
          if ((roundedDistance - coordinateUncertaintyInMeters) > ExpertDistributionOutlierTool.distanceThreshold) {
            ...
            if (!test) {
              ...
              //convert m to km
              val km = scala.math.round(outValue.get / 1000.0)
              Config.persistenceManager.put(rowKey, "occ", Map("distanceOutsideExpertRange" + Config.persistenceManager.fieldDelimiter + "p" -> km.toString()), false, false)
            }
        ...
        try {
          ...
          if (lastModifiedDate.isEmpty) {
            ...
            for (rowKey <- noLongerOutlierRowKeys) {
              ...
              Config.persistenceManager.deleteColumns(rowKey, "occ", "distanceOutsideExpertRange" + Config.persistenceManager.fieldDelimiter + "p")
